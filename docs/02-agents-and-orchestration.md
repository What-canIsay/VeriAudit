...
> **实现现状对齐**：本文为原始设计。落地时在此基础上**新增了"规模评估(Profiler)"前置环节**（计算自适应预算）与**"环境构建官(Provisioner)"角色**（deep 档搭建目标应用 + 核验预热），并把**验证官升级为 agentic 深度核验**（自主读全上下文 + sqlmap/nuclei/strace/白盒 SQL 日志等实弹复现）。编排运行时为单进程 asyncio 状态机（非 LangGraph 运行时）。差异总表见 [`README.md`](../README.md#实现现状-vs-设计文档对齐说明)，运行见 [`LAUNCH.md`](../LAUNCH.md)。
...
| 智能体 | 代号 | 一句话职责 | 关键产出 |
|---|---|---|---|
| 规模评估 Profiler | — | （前置环节）评估项目规模/复杂度，计算各阶段自适应预算上限 | 预算(步数/超时/名额…) |
| 编排官 Planner | P | 制定审计策略、采用预算、决定收敛条件 | 审计计划 |
| 侦察员 Recon | R | 识别技术栈/框架/依赖，枚举入口点与攻击面 | 项目画像、入口点清单 |
| 漏洞猎手 Hunter | H | **高召回·agentic** 产出漏洞候选（专业扫描器 + LLM 语义发现，边查边报） | 候选漏洞列表（带自评置信度） |
| 污点追踪员 Tracer | T | 对每个候选独立追踪 source→sink 污点流，判定**可达性**与净化情况 | 污点路径、可达性结论 |
| 环境构建官 Provisioner | X | （deep 档）把目标应用在沙箱里搭起来，并按项目自适应**核验预热**（测试账号/角色会话/seed） | 常驻运行环境、可复用会话与备忘 |
| 验证官 Validator | V | **agentic 独立复核**：读全上下文 + 在常驻应用上用专业工具**实弹复现** + 判定置信度 | 证据链、精确 PoC、置信度、CVSS |
