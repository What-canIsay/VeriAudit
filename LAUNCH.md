...
1. 打开前端首页 → 点击右上角 **「内置样本」**（自动填好本地路径）→ **创建并开始审计**；
   - 或点击已存在样本项目上的 **「开始审计」**。
2. 进入**审计控制台**，实时观察各智能体（规模评估 → 编排官 / 侦察员 / 漏洞猎手 / 污点追踪员 / 环境构建官 / 验证官 / 报告官）协同工作的时间线（含模型思考过程）。控制台右上角提供**运行控制**：⏸ **暂停** / ▶ **继续** / ■ **停止**，以及实时的**已运行时长**计时器。暂停/停止是**协作式**的——在下一个阶段边界或下一个模型步生效（无法在一次 LLM 调用中途硬杀线程）；停止后任务状态为「已停止」，会释放常驻沙箱容器。对应接口：`POST /api/v1/tasks/{id}/pause|resume|cancel`。
3. 审计完成后：
