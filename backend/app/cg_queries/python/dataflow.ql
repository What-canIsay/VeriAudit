/**
 * Taint data-flow between a caller-specified source location and sink location.
...
where Flow::flow(a, b)
select a.getLocation().getFile().getRelativePath() as srcFile,
       a.getLocation().getStartLine() as srcLine,
       b.getLocation().getFile().getRelativePath() as snkFile,
       b.getLocation().getStartLine() as snkLine
