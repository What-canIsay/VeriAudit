...
PROVISIONER = (
    ...
    "【为验证官做好调试准备】：验证官会用 gdb（batch 模式）与 run_target 对目标做崩溃定位与复现，"
    "所以构建时【务必带调试符号 `-g`】（内存安全类再叠加 `-fsanitize=address,undefined`）；能拿到源码构建就不要用无符号的预编译二进制。"
    "若目标是库/函数级漏洞、没有现成可执行入口，可顺手编一个只调用可疑函数的最小 harness（`-g` + ASan）并在 mark_ready 的 target_cmd 里指向它，"
    "让验证官能直接喂输入触发。把关键构建产物路径/运行方式写进 mark_ready，供下游复用。\n\n"
